...
resource "aws_key_pair" "shubhamkey" {
  key_name   = "shubhamkey"                          # Name of the key pair
  public_key = file("${path.module}/shubhamkey.pub") # Public key file for SSH access
}

# Define an AWS EC2 Instance resource
resource "aws_instance" "shubh-inst" {
  ami                    = var.AMIS[var.REGION] # Map of AMIs with region as the key
  instance_type          = "t2.micro"           # Instance type
  availability_zone      = var.ZONE1            # Availability zone
  key_name               = aws_key_pair.shubhamkey.key_name
  ...
  connection {
    ...
    private_key = file("${path.module}/shubhamkey") # Path to the private key file
